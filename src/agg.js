...
function applyFilters(bundle, filters) {
  ...
  // Un registro de "prod" (reporte de producción) no trae su propio Estado —
  // esa es una propiedad de la pieza (vive en "life", viene de TOTALIZADOR).
  // Para que el filtro de Estado también afecte las gráficas basadas en prod
  // (la mayoría) hay que resolver el Estado de cada pieza por su código y
  // aplicarlo aquí también, no solo al filtrar life.
  let estadoByCode = null;
  if (estadoSet) {
    estadoByCode = new Map();
    for (const l of bundle.life) estadoByCode.set(l[0], dict.estado[l[5]]);
  }

  const prod = bundle.prod.filter(p => {
    const [fecha, minaIdx, tipoIdx, equipoIdx, refIdx, , cm, , , operadorIdx] = p;
    ...
    if (estadoSet && !estadoSet.has(estadoByCode.get(cm))) return false;
...
// (vida completa, TOTALIZADOR) de cada pieza que se dio de baja ESE MES — no el
// avance parcial que reportó ese mes. El mes que agrupa es el de la fecha de
...
function avgMetrosPorReferenciaPorMes(bundle, life) {
  ...
  for (const [refIdx, byYm] of acc.entries()) {
    const m = new Map();
    for (const [ym, vals] of byYm.entries()) {
